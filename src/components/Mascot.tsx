        ...
        <motion.g animate={isSitting ? { y: 20, scaleY: 0.85 } : { y: 0, scaleY: 1 }}>
          <g className={(!isSitting && isVisible && !performanceMode) ? "animate-mascot-breathe" : ""}>
            ...
            <g className={isAngry ? "animate-shake" : ""}>
              ...
              {/* Glowing N Logo Badge beautifully overlaid on the Mascot's chest/body center */}
              <g transform="translate(250, 370)" className="animate-pulse">
                {/* Double layer high intensity glow backing */}
                <circle cx="0" cy="0" r="32" fill={colors.nColor} opacity="0.15" />
                <circle cx="0" cy="0" r="26" fill={colors.nColor} opacity="0.3" />
                <circle cx="0" cy="0" r="22" fill="rgba(15, 23, 42, 0.85)" stroke={colors.edge} strokeWidth="3" />
                {/* Beautiful custom-drawn bold 'N' inside the glowing capsule */}
                <g transform="translate(-13, -12)">
                  <path 
                    d="M 2,0 L 7,0 L 19,17 L 19,0 L 24,0 L 24,24 L 19,24 L 7,7 L 7,24 L 2,24 Z" 
                    fill={colors.nColor} 
                  />
                </g>
              </g>
